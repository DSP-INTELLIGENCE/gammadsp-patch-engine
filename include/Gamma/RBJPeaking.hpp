...
class RBJPeaking : public Td {
public:
    RBJPeaking(T freqHz=T(1000), T Q=T(0.707), T gainDb=T(0)){
        set(freqHz, Q, gainDb);
        reset();
    }
    ...
    void q(T qv){ mQ=std::max(qv,T(1e-6)); compute(); }
    void gainDb(T g){ mGainDb=g; compute(); }

    void set(T f,T qv,T g){
        mFreq=clampFreq(f);
        mQ=std::max(qv,T(1e-6));
        mGainDb=g;
        compute();
    }
...
private:
    T mFreq=T(1000), mQ=T(0.707), mGainDb=T(0);
    T b0,b1,b2,a1,a2;
    T s1=0,s2=0;
    ...
    void compute(){
        ...
        const T bb0= T(1)+alpha*A;
        const T bb1=-T(2)*cw;
        const T bb2= T(1)-alpha*A;
        const T aa0= T(1)+alpha/A;
        const T aa1=-T(2)*cw;
        const T aa2= T(1)-alpha/A;
