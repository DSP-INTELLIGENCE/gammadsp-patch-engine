...
class FormantPhaseHybrid : public Function
{
public:
    FormantPhaseHybrid(int stages = 4)
    : baseFreq(500.f),
      baseFB(0.35f),
      baseMix(0.7f)
    {
        ...
        chain.resize(stages);
        formants.resize(stages);

        for (int i = 0; i < stages; ++i)
        {
            chain[i].setCutoff(baseFreq);
        }
    }
    ...
    // ---------- Formant control ----------
    void setVowel(FormantMorphFilter::Voice v,
                  FormantMorphFilter::Phoneme p)
    {
        for (auto& f : formants)
        {
            f.setTargetVowel(v, p);
        }
    }
    ...
    float process(float x) override
    {
        ...
        {
            chain[i].setCutoff(cutoff);
            in = chain[i].process(in);
            in = formants[i].process(in);   // 🔥 vocal shaping per stage
        }
...
private:
    ...
    std::vector<FormantMorphFilter> formants;
