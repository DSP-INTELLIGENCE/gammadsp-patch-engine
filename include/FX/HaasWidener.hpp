...
class HaasWidener: public StereoFunction {
public:
    ...
    StereoSample process(const StereoSample& input)
    {
        ...
        dL.setDelay(std::max(0.f, half));  // delay both sides equally in magnitude
        dR.setDelay(std::max(0.f, half));  // we'll swap polarity via M/S below

        // Convert to M/S
        float M = 0.5f * (input.L + input.R);
        float S = 0.5f * (input.L - input.R);
        ...
        return StereoSample(outL * trim, outR * trim);
    }
...
private:
    ...
    Delay dL{maxDelaySeconds, 0.f};
    Delay dR{maxDelaySeconds, 0.f};
