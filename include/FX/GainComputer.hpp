...
public:
    ...
    // Configuration
    void setThreshold(Sample t) { mThreshold = std::max((Sample)1e-6, t); }
    ...
    // Core: compute gain from control level (not audio sample)
    Sample process(Sample level)
    {
        level = std::max(level, (Sample)1e-9);
        Sample g = computeGain(level);
        mGain = g;
        return g;
    }

    Sample gain() const { return mGain; }
